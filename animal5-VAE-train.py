...
from utils.losses import BVAE_loss
from utils.models import TwoPartResNetVAE
...
def training_loop(model, criterion, optimizer, train_loader, valid_loader, epochs, device, dir_out, print_every=1):
    ...
    plot_losses(train_losses, valid_losses, train_accuracies, val_accuracies, out_dir)
...
def train(train_loader, model, criterion, optimizer, device):
    ...
    for i, (X, y_true) in tqdm(enumerate(train_loader), total=len(train_loader)):
        ...
        # Forward pass
        mu, logvar, y_hat, x_reconst = model(X)
        recon_loss, kl_div = criterion[0](X, x_reconst, mu, logvar)
        loss0 = recon_loss + lambda_corr * kl_div
        loss1 = criterion[1](y_hat, y_true)
        loss = loss0 + loss1
    ...
    accuracy = (correct_pred.float() / n).item().cpu()
...
out_dir = f"animal5-VAE/resnet18-{lambda_corr}/"
if not os.path.exists(out_dir):
    os.makedirs(out_dir)
model = TwoPartResNetVAE(classes=len(a10_train.classes))
model.to(DEVICE)

if not os.path.exists(os.path.join(out_dir, "last.pt")):
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)
    criterion1 = BVAE_loss
...
for i, row in enumerate(index_per_feat):
    if not os.path.exists(f"animal5-VAE-results/results-{lambda_corr}/{i}/"):
        os.makedirs(f"animal5-VAE-results/results-{lambda_corr}/{i}/")
    start = time.time()
    weight = model.cl.linear.weight.cpu().detach().numpy()
    fig, ax = visualize_weights(weight, i, a10_val.classes)
    fig.savefig(f"animal5-VAE-results/results-{lambda_corr}/{i}/weight_plot.png")
    plt.close(fig)
    class_inst = {}
    ...
    plt.savefig(f"animal5-VAE-results/results-{lambda_corr}/{i}/most_activated.png")
    plt.close()
    row = row[:12]
    for j, col in enumerate(row.tolist()):
        ...
        cv2.imwrite(f"animal5-VAE-results/results-{lambda_corr}/{i}/{j}-{col}.png",
