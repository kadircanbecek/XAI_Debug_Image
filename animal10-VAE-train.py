...
out_dir = f"animal10-VAE/resnet18-{lambda_corr}/"
...
for i, row in enumerate(index_per_feat):
    if not os.path.exists(f"animal10-VAE-results/results-{lambda_corr}/{i}/"):
        os.makedirs(f"animal10-VAE-results/results-{lambda_corr}/{i}/")
    start = time.time()
    weight = model.cl.linear.weight.cpu().detach().numpy()
    fig, ax = visualize_weights(weight, i, a10_val.classes)
    fig.savefig(f"animal10-VAE-results/results-{lambda_corr}/{i}/weight_plot.png")
    plt.close(fig)
    class_inst = {}
    ...
    plt.savefig(f"animal10-VAE-results/results-{lambda_corr}/{i}/most_activated.png")
    plt.close()
    row = row[:12]
    for j, col in enumerate(row.tolist()):
        ...
        cv2.imwrite(f"animal10-VAE-results/results-{lambda_corr}/{i}/{j}-{col}.png",
