...
dataset = datasets.EuroSAT(root='./data',
                           download=True, transform=transform)

# valid_dataset = datasets.EuroSAT(root='./data',
#                                  download=True,
#                                  transform=transform)
...
valid_loader = DataLoader(dataset=test_ds,
                          batch_size=BATCH_SIZE,
                          shuffle=False)
...
if not os.path.exists("eurosat.pt"):
    model = Net().to(DEVICE)
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)
    criterion = nn.CrossEntropyLoss()

    model, optimizer, _ = training_loop(model, criterion, optimizer, train_loader, valid_loader, N_EPOCHS, DEVICE)
    torch.save(model.state_dict(), "eurosat.pt")

modeltoeval = Net()
modeltoeval.load_state_dict(torch.load("eurosat.pt"))
modeltoeval.eval()
