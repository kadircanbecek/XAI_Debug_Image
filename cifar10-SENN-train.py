...
from utils.models import TwoPartResNetSENN, TwoPartConvSENN
...
def training_loop(model, criterionVAE, criterionCl, criterionRobust, optimizer, train_loader, valid_loader, epochs,
    ...
    plot_losses(train_losses, valid_losses, train_accuracies, valid_accuracies, dir_out)
...
out_dir = f"cifar10-SENN/resnet18-{robust_reg}/"
if not os.path.exists(out_dir):
    os.makedirs(out_dir)
model = TwoPartConvSENN(classes=len(train_dataset.classes))
model.to(DEVICE)
...
if not os.path.exists(os.path.join(out_dir, "last.pt")):
    ...
    # validate(valid_loader, model, criterionVAE, criterionCL, criterionRobust, DEVICE)
...
#     if not os.path.exists(f"cifar10-SENN-results/results-{robust_reg}/{i}/"):
#         os.makedirs(f"cifar10-SENN-results/results-{robust_reg}/{i}/")
#     start = time.time()
#     weight = model.cl.linear.weight.cpu().detach().numpy()
#     fig, ax = visualize_weights(weight, i, a10_val.classes)
#     fig.savefig(f"cifar10-SENN-results/results-{robust_reg}/{i}/weight_plot.png")
#     plt.close(fig)
#     class_inst = {}
...
#     plt.savefig(f"cifar10-SENN-results/results-{robust_reg}/{i}/most_activated.png")
#     plt.close()
#     row = row[:12]
...
# 
#         cv2.imwrite(f"cifar10-SENN-results-{robust_reg}/{i}/{j}-{col}.png",
